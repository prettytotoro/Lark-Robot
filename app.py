...
from core.scraper import login_session, fetch_raw


def render_field_candidates(response_type, parsed, key_prefix):
    """
    渲染字段候选UI(单值字段勾选加入 + 明细列表一键设置)。
    抽成公共函数，因为"上传样本"和"一键测试抓取"两个入口都要用同一套渲染逻辑。
    """
    if response_type == "json":
        leaves, array_cands, dict_cands = detect_json_fields(parsed)

        st.write(f"识别到 {len(leaves)} 个单值字段候选：")
        chosen_idx = []
        for i, leaf in enumerate(leaves):
            c1, c2 = st.columns([1, 4])
            checked = c1.checkbox("选", key=f"{key_prefix}_leafchk_{i}", label_visibility="collapsed")
            c2.write(f"`{leaf['path']}` → 建议命名 **{leaf['key_suggestion']}**，样例值: `{leaf['value_preview']}`")
            if checked:
                chosen_idx.append(i)
        if st.button("✅ 把勾选的单值字段加入字段列表", key=f"{key_prefix}_addleaf"):
            ...
            st.success(f"已加入 {len(chosen_idx)} 个字段")
            st.rerun()
        ...
        if array_cands:
            ...
            for i, arr in enumerate(array_cands):
                ...
                if st.button("设为明细列表", key=f"{key_prefix}_setarr_{i}"):
        ...
        if dict_cands:
            ...
            for i, dc in enumerate(dict_cands):
                ...
                if st.button("设为明细列表", key=f"{key_prefix}_setdict_{i}"):
        ...
        leaf_cands, row_cands = detect_html_fields(parsed)
        ...
        for i, leaf in enumerate(leaf_cands):
            c1, c2 = st.columns([1, 4])
            checked = c1.checkbox("选", key=f"{key_prefix}_htmlleafchk_{i}", label_visibility="collapsed")
            c2.write(f"`{leaf['path']}` → 建议命名 **{leaf['key_suggestion']}**，样例值: `{leaf['value_preview']}`")
            if checked:
                chosen_idx.append(i)
        if st.button("✅ 把勾选的单值字段加入字段列表", key=f"{key_prefix}_addhtmlleaf"):
            existing_paths = {f["path"] for f in st.session_state["bot_fields"]}
            for i in chosen_idx:
                leaf = leaf_cands[i]
                if leaf["path"] not in existing_paths:
        ...
        if row_cands:
            ...
            for i, row in enumerate(row_cands):
                ...
                if st.button("设为明细列表(HTML)", key=f"{key_prefix}_sethtmlrow_{i}"):
...
with tab_bots:
    ...
    with right:
        ...
        if editing_id == "__unset__":
            ...
            with st.expander("🚀 一键测试抓取（自动登录+抓取+识别字段，推荐）", expanded=True):
                st.caption(
                    "直接用上面①②步填的登录信息和目标URL，让应用自己登录、自己抓、自己识别字段，你只需要勾选。"
                    "如果目标网站只能在公司内网/VPN访问，这个应用(部署在云端)连不上，那就用下面「手动上传样本」的方式。"
                )
                if st.button("开始测试抓取", type="primary"):
                    try:
                        with st.spinner("登录中..."):
                            session = login_session(login_cfg)
                        with st.spinner("抓取数据中..."):
                            resp_type, parsed = fetch_raw(session, {
                                "url": fetch_url, "method": fetch_method, "response_type": response_type,
                            })
                        st.session_state["live_fetch_result"] = (resp_type, parsed)
                        st.success("抓取成功，识别结果如下 👇")
                    except Exception as e:
                        st.session_state.pop("live_fetch_result", None)
                        st.error(f"抓取失败: {e}")
                        st.caption("常见原因：登录方式配错了(表单字段名不对)、URL不对、或者这个网站只能内网访问连不到。")

                if "live_fetch_result" in st.session_state:
                    resp_type, parsed = st.session_state["live_fetch_result"]
                    render_field_candidates(resp_type, parsed, key_prefix="live")

            with st.expander("🔍 手动上传样本自动识别字段（网站连不上时的备用方案）", expanded=False):
                st.caption(
                    "如果目标网站只能在公司内网访问，云端应用连不上，就用这个："
                    "自己登录网站，F12开发者工具把数据接口的Response复制出来存成 .json 文件（或网页另存为 .html）上传上来。"
                )
                ...
                if sample_file is not None:
                    ...
                    if is_json:
                        ...
                        render_field_candidates("json", parsed_sample, key_prefix="upload")
                    else:
                        soup = BeautifulSoup(raw_bytes.decode("utf-8", errors="ignore"), "html.parser")
                        render_field_candidates("html", soup, key_prefix="upload")
